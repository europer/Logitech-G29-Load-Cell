...
void calibrate()
{
    ...
    SerialBT.println("***");
    SerialBT.println("give in max break reduction factor in % and hit return. -1 without factor no changes");
    SerialBT.println("***");
    ...
    while (_resume == false)
    {
        if (Serial.available() > 0 || SerialBT.available())
        {
            ...
            {
                max_break_redfac = temp_redfacBT;
                _resume = true;
            }
        }
    }
...
void voltage_cali()
{
    ...
    Serial.println("***");
    Serial.println("Current Max Volt bit/Volt: ");
    Serial.print(max_volt);
    Serial.print("/");
    ...
    SerialBT.println("***");
    SerialBT.println("Current Max Volt bit/Volt: ");
    SerialBT.print(max_volt);
    SerialBT.print("/");
    SerialBT.println(max_volt * ref_voltage);
    ...
    SerialBT.println("SerialBT Command is '-2' for good or '-1' not to change this value");
    SerialBT.println("***");
    ...
    while (_resume == false)
    {
        ...
        if (SerialBT.available())
        {
            ...
            Serial.println("bt temp voltage test: ");
            Serial.print(temp_voltBT);

            if (temp_voltBT == -2)
            {
                max_volt = temp_volt_maxBT;
                _resume = true;
            }
            ...
            {
                temp_volt_maxBT = temp_voltBT;

                dacWrite(DAC1, temp_voltBT); //sending voltage to break and look at TV/Monitor to find max Break

                Serial.println("Temp max VoltBT bit/Volt: ");
                Serial.print(temp_voltBT);
                Serial.print("/");
                Serial.print(temp_voltBT * ref_voltage);
                Serial.println("");

                SerialBT.println("Temp max VoltBT bit/Volt: ");
        ...
        if (Serial.available() > 0)
        {
            ...
            Serial.println("temp voltage test: ");
            ...
            else if (inByte.toInt() >= 0 && inByte.toInt() <= 255)
            {
                ...
                Serial.println("Temp max Volt bit/Volt: ");
                Serial.print(temp_volt);
                Serial.print("/");
                Serial.print(temp_volt * ref_voltage);
                Serial.println("");

                SerialBT.println("Temp max Volt bit/Volt: ");
    ...
    Serial.println("***");
    Serial.println("Current min Volt bit/Volt: ");
    Serial.print(min_volt);
    Serial.print("/");
    Serial.println(min_volt * ref_voltage);
    Serial.println("Start calibration MIN voltate to get 100% breaking:");
    ...
    SerialBT.println("***");
    SerialBT.println("Current min Volt bit/Volt: ");
    SerialBT.print(min_volt);
    SerialBT.print("/");
    SerialBT.println(min_volt * ref_voltage);
    SerialBT.println("Start calibration MIN voltate to get 100% breaking:");
    SerialBT.println("Adjust the voltage up or down to get min breaking (100%)");
    SerialBT.println("Values are given in a range between 0 and 255, the voltage Value will be displayed");
    SerialBT.println("SerialBT Command is '-2' for good or '-1' not to change this value");
    SerialBT.println("***");
    ...
    while (_resume == false)
    {
        ...
        if (Serial.available() > 0)
        {
            ...
            else if (inByte.toInt() >= 0 && inByte.toInt() <= 255)
            {
                ...
                Serial.println("Temp min Volt bit/Volt: ");
                Serial.print(temp_volt);
                Serial.print("/");
                Serial.print(temp_volt * ref_voltage);
                Serial.println("");

                SerialBT.println("Temp min Volt bit/Volt: ");
                SerialBT.print(temp_volt);
                SerialBT.print("/");
                SerialBT.print(temp_volt * ref_voltage);
                SerialBT.println("");
        ...
        else if (SerialBT.available())
        {
            ...
            else if (temp_voltBT >= 0 && temp_voltBT <= 255)
            {
                ...
                Serial.println("Temp min VoltBT bit/Volt: ");
                Serial.print(temp_voltBT);
                Serial.print("/");
                Serial.print(temp_voltBT * ref_voltage);
                Serial.println("");

                SerialBT.println("Temp min VoltBT bit/Volt: ");
                SerialBT.print(temp_voltBT);
                SerialBT.print("/");
                SerialBT.print(temp_voltBT * ref_voltage);
                SerialBT.println("");
