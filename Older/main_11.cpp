...
void load_variables_eeprom(int ja_nein)
{
    ...
    if (ja_nein == 1) //from EEPROM = 1, and from RAM = 0
    {
        ...
        EEPROM.get(temp_adress, gamma); //get also max reduced break factor
    }
...
void normalisation()
{
    ...
    //gamma factor
    calicalulation_break(gammafac, 0, 0, 2);
    Serial.flush();   //clean buffer
    SerialBT.flush(); //clean buffer
    ...
    while (_resume == false)
    {
        ...
        if (Serial.available() > 0 || SerialBT.available())
        {
            ...
            if (inByte == 'y' || BTByte == 'y')
            {
                ...
                temp_adress += sizeof(normal);
                EEPROM.put(temp_adress, gamma); //get also max reduced break factor
...
void loop()
{
    ...
    else
    {
        ...
        float weight_in_percent = mapping(gi, min_break, max_break, 0.0, 1.0); //mapping into %
        ...
        {
            for (int n = (laod_percent_steps - 1); n >= 1; n--) // n=0 =>100% & n=78 => 0.0% zero break, 79 Steps
            {
        ...
        else if (weight_in_percent == 0.0 || weight_in_percent == 1.0) // for 0 or 1 we do not need to check
        {
            ...
            {
                lower_band_case = (laod_percent_steps - 1); //78
        ...
        float lower_weighted_percent;
        float upper_weighted_percent;
        float delta;
        int lower_pwm;
        int upper_pwm;
        int pwm = 10.0;
        ...
        if (delta != 0.0)
        {
            ...
            lower_pwm = pwm * round(lower_weighted_percent); // example 80% gives 8 times PWM
            upper_pwm = pwm * round(upper_weighted_percent); // example 20% give 2 times PWM
        }
        else
        {
        ...
        float lower_bit_case = round(mapping(lower_band_case, (laod_percent_steps - 1), 0, min_break_volt, max_break_volt));
        float upper_bit_case = round(mapping(upper_band_case, (laod_percent_steps - 1), 0, min_break_volt, max_break_volt));

        if (lower_bit_case < 0)
            lower_bit_case = 0;
        if (lower_bit_case > 255)
            lower_bit_case = 255;
        if (upper_bit_case < 0)
            upper_bit_case = 0;
        if (upper_bit_case > 255)
            upper_bit_case = 255;

        if (lower_pwm == 0 && upper_pwm == 0) //max or min break
        {
            if (volt_direction_normal == true)
            {
                int lbc_max = lower_bit_case + 2; //give 2 bit more for max
                int lbc_min = lower_bit_case - 2; //give 2 bit less for min
                if (lbc_min < 0)
                    lbc_min = 0;
                if (lbc_max > 255)
                    lbc_max = 255;

                if (max_break == lower_bit_case) // is max break
                    dacWrite(DAC1, lbc_max);
                else // is min break
                    dacWrite(DAC1, lbc_min);
            }
            else
            {
                int lbc_max = lower_bit_case - 2; //give 2 bit less for max
                int lbc_min = lower_bit_case + 2; //give 2 bit more for min
                if (lbc_min < 0)
                    lbc_min = 0;
                if (lbc_max > 255)
                    lbc_max = 255;
        ...
        else if ((lower_pwm == 0 && upper_pwm > 0) || (lower_pwm > 0 && upper_pwm == 0))
        {
            if (lower_pwm == 0)
        ...
        if (millis() > t + serialPrintInterval)
        {
            if (SerialPrintData == 1)
            {
                ...
                Serial.print("  GLED: ");
                Serial.print(lower_bit_case);
                Serial.print("/");
                Serial.print((lower_bit_case / 255.0) * 3.30);
                Serial.print(" Volt");
