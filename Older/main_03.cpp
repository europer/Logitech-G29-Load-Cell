...
float max_break = 20.00f;
float min_break = 0.00f;
...
void calibrate()
{
  ...
  if (known_mass != 0)
  {
    ...
    max_break = LoadCell.getData() / 1000.0; //give in in Gram but putput in Kg
  }
  else if (known_mass == 0)
  {
...
#endif
    int temp_adress = 0;
    temp_adress += EEPROM.get(calVal_eepromAdress, newCalibrationValue); // uncomment this if you want to fetch the calibration value from eeprom

    LoadCell.setCalFactor(newCalibrationValue);                        // set calibration value (float)
    temp_adress += EEPROM.get(sizeof(newCalibrationValue), max_break); //Save also max break
  }
...
#endif
        int temp_adress = 0;
        temp_adress += EEPROM.put(calVal_eepromAdress, newCalibrationValue);
        temp_adress + EEPROM.put(sizeof(newCalibrationValue), max_break); //Save also max break
#if defined(ESP8266) || defined(ESP32)
        EEPROM.commit();
#endif
...
void loop()
{
  ...
  if (newDataReady)
  {
    if (millis() > t + serialPrintInterval)
    {
      ...
      float gi = i;
      if (gi > max_break * 0.80)
        gi = max_break * 0.80;

      float GLED = mapping(gi, 0.0, max_break * 0.80, 0.0, 255.0);
      Serial.print("  GLED: ");
      Serial.print(GLED);
      ...
      {
        float RLED = mapping(ri, max_break * 0.80, max_break, 0.0, 255.0);
  ...
  if (Serial.available() > 0)
  {
    ...
    if (inByte == 't')
      ...
      min_break = LoadCell.getData() / 1000.0; //give in in Gram but putput in Kg
  }
