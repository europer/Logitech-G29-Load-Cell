...
void SerialPrintOutCollector(long c, int lc_out, float mapped_voltage, bool nmal, float gfac, float wiper, int outputtype)
{
    ...
    else if (outputtype == 2)
    {
        ...
        print_serial_and_bt(String(wiper * 100.0), 0);
                                ...
                                int &dac_case)
{
    ...
    weight_in_percent = mapping(loadcellcleaned, min_break, max_break, 0.0, 1.0); //mapping into %
    ...
    if (lower_bit_case == min_break_volt || lower_bit_case == max_break_volt) //max or min break
    {
        if (volt_direction_normal == true)
        {
            ...
            {
                lower_bit_case = min_break_volt - 2; //give 2 bit less at the min/max ends
                upper_bit_case = min_break_volt - 2; //give 2 bit more at the min/max ends
            ...
            {
                lower_bit_case = max_break_volt + 2; //give 2 bit more at the min/max ends
                upper_bit_case = max_break_volt + 2; //give 2 bit less at the min/max ends
                //security check that we have right range
                bitcheckint(lower_bit_case, minbit, maxbit);
                bitcheckint(upper_bit_case, minbit, maxbit);
                dac_case = 2;
